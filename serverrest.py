...
def indexPage():
	...
	print (num)
	return render_template("indexrest.html",num=num)

@app.route('/queue')
def queuePage():
	return render_template("queuerest.html")
...
def custPage(order_num):
	...
	c.execute(query1)
	data=c.fetchall()
	return render_template('custrest.html',data=data,order_num=order_num)

@app.route('/order/<order_num>',methods=['GET'])
...
@app.route('/order/<order_num>/<item_num>',methods=['POST'])
